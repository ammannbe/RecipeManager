...
class IngredientAlternativesTest extends TestCase
{
    ...
    public function test_the_edit_form_nests_alternatives_under_their_parent(): void
    {
        ...
        $this->assertEquals($parent->food_id, $row['food_id']);
        $this->assertCount(1, $row['ingredients']);
        $this->assertEquals(
            $alternative->food_id,
            reset($row['ingredients'])['food_id'],
        );
    }

    public function test_an_alternative_can_be_added_through_the_form(): void
    {
        $user = User::factory()->admin()->create();
        $recipe = Recipe::factory()->create(['author_id' => $user->author_id]);
        $parent = $this->parent($recipe);
        ...
        $key = array_key_first($data['ungroupedIngredients']);

        $data['ungroupedIngredients'][$key]['ingredients'] = [
            'new-1' => [
                'amount' => 2,
                'amount_max' => null,
                'unit_id' => null,
        ...
        $component->set('data', $data)->call('save')->assertHasNoFormErrors();
